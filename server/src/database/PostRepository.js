...
class PostRepository {
  ...
  async find(id_post) {
    ...
    SELECT p.id_post,p.description, img.url_image
    FROM posts p
  ...
  async get({ tag, expression, cursor }) {
    ...
    if (tag || expression) {
      let list = []
      const ex = tag?.toLowerCase() || expression?.toLowerCase()
      list.push(`'${ex}' = ANY (p.tags) or p.description ilike '%${ex}%'`)
    ...
    const query = `
          SELECT p.id_post,concat(us.first_name,' ',us.last_name) as author_name,p.description, p.upload_date,img.url_image,avt.url as author_avatar,p.tags, array_agg(distinct psus.id_user) as likes
          FROM posts p 
          ...
          group by p.id_post,us.first_name,us.last_name,p.description,p.tags,p.upload_date,img.url_image,avt.url
