...
const resgiterUser = async (req, resp) => {
  try {
    ...
    RepositoryAvatarsUser.insertAvatar(insertId_user);

    const access_token = generateToken({ id_user: insertId_user, ...req.body });

    return resp.status(200).json({
      succes: true,
      id_user: insertId_user,
      avatar: {
        url: "https://ik.imagekit.io/picmont/icons/default_avatar.png?updatedAt=1687206611943",
        id_cnd: 0,
      },
